...
public class FlutterOverlayWindowPlugin implements
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull Result result) {
        if (call.method.equals("checkPermission")) {
            ...
            if (mActivity == null) {
                result.error("NO_ACTIVITY", "Cannot request permission without a foreground activity", null);
                return;
            }
            ...
            if (x == null || y == null) {
                result.error("INVALID_ARGS", "moveOverlay requires x and y", null);
                return;
            }
            ...
            Boolean enabledArg = call.argument("enabled");
            Integer zoneWidthArg = call.argument("zoneWidthPx");
            boolean enabled = enabledArg != null ? enabledArg : false;
            int zoneWidthDp = zoneWidthArg != null ? zoneWidthArg : 0;
    ...
    // Create the overlay engine on demand, on the first showOverlay when the cache is empty.
    // This avoids spinning up the engine on every activity attach, before permission is granted.
    // OverlayService.onCreate keeps its own fallback creation for the case where this has not run.
