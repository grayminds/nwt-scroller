...
class _ScrollOverlayState extends State<ScrollOverlay>
  ...
  int get _expandedHeight {
    if (_config.interactionStyle == InteractionStyle.minimalFade) {
  ...
  Future<void> _init() async {
    ...
    // Tell the main app the overlay engine finished its first load, so it can
    // push the current config now instead of guessing with a fixed delay.
    try {
      await OverlayService.shareData({'type': 'ready'});
    } catch (_) {}
  }
  ...
  void _onDataReceived(dynamic data) {
    ...
    if (data is Map) {
      ...
      if (type == 'config') {
        ...
        _onScreenSizeChanged(data['w'], data['h']);
      }
    }
  }

  /// Apply a native rotation update.  The width and height arrive in physical
  /// pixels; config screen dimensions are logical pixels, so divide by the
  /// device pixel ratio before storing them.  Re-run the expanded layout when
  /// expanded so the new bounds take effect immediately.
  void _onScreenSizeChanged(dynamic w, dynamic h) {
    if (w is! num || h is! num || w <= 0 || h <= 0) return;
    final dpr = ScreenService.getDevicePixelRatio();
    if (dpr <= 0) return;
    _config.screenWidth = w / dpr;
    _config.screenHeight = h / dpr;
    if (_expanded) {
      _resizeExpanded();
    }
  }

  ScrollTheme _resolveTheme(ScrollThemeId id) {
    switch (id) {
      case ScrollThemeId.silver:
        return SilverTheme();
      case ScrollThemeId.blue:
        return BlueTheme();
      case ScrollThemeId.parchment:
