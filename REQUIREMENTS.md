...
- History is shown in the main app's History tab.  (The overlay swipe-up popup was cut in the 2026-07-14 alpha review;  see `docs/alpha-review-corrections.md` items 2.1 and 2.2.)
...
### FR-12: Drag-to-Dismiss (Collapsed State)

- While the collapsed overlay is dragged, a circular close target with an X appears at bottom center of the screen.
- The target is purely visual (not touchable); hit-testing uses the drag pointer in screen coordinates with a forgiving magnetic radius (about 1.8x the visual radius).
- When the pointer enters the target, the target arms:  it enlarges and turns red.
- Releasing the drag over the armed target stops the overlay service, so the overlay closes.  Releasing elsewhere hides the target and runs the normal edge-snap animation.
- The target is torn down on drag end and on service destroy.  It is implemented natively in `OverlayService` as a second lightweight window, with no additional Flutter engine.
